...
void mk_fg_node_gibs_resources(Event*, ComponentsAccessor&)
{
  ...
  { 
    ...
    auto gibs_surfels_coverage = reg.createTexture("gibs_surfels_coverage",
      gapi::TextureAllocationDescription{
    ...
    return [gibs_dbg_alloc,gibs_surfels_lifetime,gibs_surfels_storage,gibs_surfels_pool,gibs_surfels_allocation_locks,gibs_surfels_spatial_storage,gibs_surfels_meta,gibs_surfels_coverage,gibs_surfels_sdf,gibs_surfels_allocation_pos](gapi::CmdEncoder& encoder)
    {
      gibs_resources_init(encoder, gibs_dbg_alloc.get(), gibs_surfels_lifetime.get(), gibs_surfels_storage.get(), gibs_surfels_pool.get(), gibs_surfels_allocation_locks.get(), gibs_surfels_spatial_storage.get(), gibs_surfels_meta.get(), gibs_surfels_coverage.get(), gibs_surfels_sdf.get(), gibs_surfels_allocation_pos.get());
    };
  });
}
...
//Engine::OnFrameGraphInit handler
static
void mk_fg_node_gibs_surfels_coverage(Event*, ComponentsAccessor&)
{
  fg::register_node("gibs_surfels_coverage", FG_FILE_DECL, [](fg::Registry& reg)
  { 
    const uint2 __renderSize__ = reg.getRenderSize();
    ...
    auto surfelsCoverage = reg.modifyTexture("gibs_surfels_coverage", gapi::TextureState::ShaderReadWrite);
    ...
    return [gbuffer_depth,surfelsCoverage,surfelsSDF,surfelsStorage,surfelsSpatialStorage,render_size](gapi::CmdEncoder& encoder)
    {
      tfx::set_extern("gbuffer_depth", gbuffer_depth.get());
      tfx::set_extern("surfelsCoverage", surfelsCoverage.get());
      tfx::set_extern("surfelsSDF", surfelsSDF.get());
      tfx::set_extern("surfelsStorage", surfelsStorage.get());
      tfx::set_extern("surfelsSpatialStorage", surfelsSpatialStorage.get());
      gibs_surfels_coverage(encoder, render_size.get());
    };
  });
}

static
EventSystemRegistration mk_fg_node_gibs_surfels_coverage_registration(
  mk_fg_node_gibs_surfels_coverage,
  compile_ecs_name_hash("OnFrameGraphInit"),
  {
  },
  "mk_fg_node_gibs_surfels_coverage"
...
void mk_fg_node_gibs_surfels_find_surfels_alloc_pos(Event*, ComponentsAccessor&)
{
  ...
  { 
    ...
    fg::dsl::AccessDecorator render_size{__renderSize__};
    auto surfelsSDF = reg.readTexture("gibs_surfels_sdf", gapi::TextureState::ShaderRead, false);
    auto surfelsAllocPos = reg.modifyTexture("gibs_surfels_allocation_pos", gapi::TextureState::ShaderReadWrite);

    return [surfelsSDF,surfelsAllocPos,render_size](gapi::CmdEncoder& encoder)
    {
      tfx::set_extern("surfelsSDF", surfelsSDF.get());
...
void mk_fg_node_gibs_allocate_surfels(Event*, ComponentsAccessor&)
{
  ...
  { 
    ...
    auto surfelsCoverage = reg.readTexture("gibs_surfels_coverage", gapi::TextureState::ShaderRead, false);
    ...
    return [dbgTex,gbuffer_normal,gbuffer_depth,surfelsLifeTime,surfelsStorage,surfelsPool,surfelsAllocLocks,surfelsMeta,surfelsCoverage,surfelsAllocPos,render_size](gapi::CmdEncoder& encoder)
    {

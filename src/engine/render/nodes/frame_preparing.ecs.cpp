...
ECS_EVENT_SYSTEM()
static
void mk_backbuffer_acquiring_node(const Engine::OnFrameGraphInit&)
{
...
ECS_EVENT_SYSTEM()
static
void mk_frame_preparing_node(const Engine::OnFrameGraphInit&)
{
  fg::register_node("frame_preparing", FG_FILE_DECL, [](fg::Registry& reg)
  {
    ...
    {
      wndSize.get() = Engine::Render::get_render_size();
      ...
      cameraData.get() = Engine::get_camera(aspect, float3(cameraJitter, 0.0));

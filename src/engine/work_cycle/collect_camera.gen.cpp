...
using namespace Engine::ECS;
  ...
  const static query_id query_camera_queryId = Registry::registerDirectQuery(
    DirectQueryDescription{
      .components = {
         DESCRIBE_QUERY_COMPONENT("pos", float3), DESCRIBE_QUERY_COMPONENT("camera_fov", float), DESCRIBE_QUERY_COMPONENT("camera_zNear", float), DESCRIBE_QUERY_COMPONENT("camera_zFar", float)
      }
    }
  );
  

  static void query_camera (eastl::function<void( const float3& pos, const float& camera_fov, const float& camera_zNear, const float& camera_zFar)> cb)
  {
 ...
 const float& camera_zFar = accessor.get<float>(str_hash("camera_zFar")) ;
      cb(pos,camera_fov,camera_zNear,camera_zFar);
    });
  }
